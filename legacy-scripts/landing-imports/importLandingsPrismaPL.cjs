#!/usr/bin/env node
/* Polish landing-page importer — a separate copy of the DE importer (leaves the DE
   pipeline untouched). Reads scripts/pl-landings.csv (one row per NEW Polish page) +
   companion pl-content/*.md, builds the 6-block commercial template, and upserts a
   DRAFT `singlepage` row (language "pl"). Paired to the EN page via translationGroupId.
   ...
   Usage (on a host with the prod DB + prisma client):
     ...
     node scripts/importLandingsPrismaPL.cjs --write --only=<slug-pl>
...
function buildBlocks(row, c) {
  const blocks = [];
  blocks.push({ _key: k("b-intro-pl"), _type: "landingIntroBlock", image: img(row.image_ref, row.intro_title_pl), title: row.intro_title_pl, subtitle: row.intro_subtitle_pl, buttonLabel: row.intro_button_pl || "Zobacz projekty", description: row.intro_desc_pl });
  const proj = { _key: k("b-proj-pl"), _type: "landingProjectsBlock", title: row.projects_title_pl };
  if (row.projects_city) proj.filterCity = row.projects_city;
  if (row.projects_type) proj.filterPropertyType = row.projects_type;
  blocks.push(proj);
  blocks.push({ _key: k("b-tf-pl"), _type: "landingTextFirst", content: mdToPT(c.bodyFirst) });
  const items = c.faqItems.map((it, i) => ({ _key: k(`faq-pl-${i}`), question: it.question, answer: mdToPT(it.answerMd) }));
  blocks.push({ _key: k("b-faq-pl"), _type: "landingFaqBlock", title: row.faq_title_pl || "Najczęściej zadawane pytania", faq: { _type: "accordionBlock", items } });
  blocks.push({ _key: k("b-ts-pl"), _type: "landingTextSecond", content: mdToPT(c.bodySecond) });
  return blocks;
}

async function run() {
  const rows = parseCSV(fs.readFileSync(path.resolve(__dirname, "pl-landings.csv"), "utf8"));
  console.log(`${rows.length} row(s) in pl-landings.csv | mode: ${WRITE ? "WRITE" : "DRY RUN"}${ONLY ? ` | only=${ONLY}` : ""}\n`);
  for (const row of rows) {
    if (!row.slug_pl) continue;
    if (ONLY && row.slug_pl !== ONLY) continue;
    let tgid = null;
    if (row.en_slug) {
      const en = await prisma.singlepage.findFirst({ where: { language: "en", slug: row.en_slug } });
      if (!en) { console.log(`⚠ ${row.slug_pl}: EN page "${row.en_slug}" not found — skipping`); continue; }
    ...
    const relSlugs = String(row.related_slugs_pl || "").split(",").map((s) => s.trim()).filter(Boolean);
    const related = [];
    for (const rs of relSlugs) {
      const r = await prisma.singlepage.findFirst({ where: { language: "pl", slug: rs }, select: { sanityId: true } });
      if (r) related.push({ _key: k("rel"), _ref: r.sanityId, _type: "singlepageRef" });
    }
    const content = parseContentFile(row.content_file);
    const sanityId = `local-pl-${row.slug_pl}`;
    const data = {
      sanityId, translationGroupId: tgid, language: "pl", slug: row.slug_pl,
      title: row.intro_title_pl, excerpt: row.excerpt_pl || row.meta_desc_pl,
      allowIntroBlock: false, previewImage: img(row.image_ref, row.meta_title_pl),
      seo: { metaTitle: row.meta_title_pl, metaDescription: row.meta_desc_pl },
      contentBlocks: buildBlocks(row, content), relatedLandingPages: related,
    };
    const blocks = data.contentBlocks;
    console.log(`• ${row.slug_pl}  [EN:${row.en_slug || "-"}]  faq:${blocks.find((b) => b._type === "landingFaqBlock").faq.items.length}  bodyFirst:${blocks.find((b) => b._type === "landingTextFirst").content.length}pt  related:${related.length}  filter:${row.projects_city || "-"}/${row.projects_type || "-"}`);
    if (WRITE) {
