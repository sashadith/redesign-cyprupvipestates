...
- `OPENWA_BASE_URL` — the OpenWA gateway origin: `http://127.0.0.1:2785`. The gateway
  runs on the same VPS as this app; traffic never leaves the machine and never passes through nginx.
- `OPENWA_API_KEY` — a dedicated operator-role API key, minted separately from the one
  Claude Code uses so either can be revoked without breaking the other. Generated by the
  operator with the ssh command documented below.
- `OPENWA_SESSION_ID` — the WhatsApp session UUID, obtained from `GET /api/sessions` on
  the OpenWA gateway.
- `OPENWA_DAILY_SEND_CAP` — maximum WhatsApp messages to send per day: `20`. The counter
  resets at midnight **Cyprus time** (UTC+2, or UTC+3 during daylight saving), and counts
  only messages sent through `crm_whatsapp_send`; manual WhatsApp logs in the admin do not count.

All four WhatsApp variables must exist in `/var/www/shared/.env` **before** deploying
the code, or the two new tools will return a config error naming the missing variable.
The tools will be registered and will appear in the claude.ai connector, but they will fail
loudly on any call until all four are set.
...
4. WhatsApp setup (Phase 4 only): `OPENWA_BASE_URL`, `OPENWA_SESSION_ID`, and `OPENWA_DAILY_SEND_CAP` in `/var/www/shared/.env`. To mint `OPENWA_API_KEY`, run on the VPS:
   ```bash
   ...
   ```
   Copy the returned `apiKey` value into `OPENWA_API_KEY` in the `.env`.
5. Crontab: `15 5 * * * curl -s "http://127.0.0.1:3000/api/cron/mcp-cleanup?key=$CRON_SECRET"`.
6. Crontab: `50 4 * * * curl -s "http://127.0.0.1:3000/api/cron/inventory-snapshot?key=$CRON_SECRET"` (after drive-sync 04:30, before action-digest 05:00). Trigger it once by hand right after the first deploy so history starts on day one: `curl -s "http://127.0.0.1:3000/api/cron/inventory-snapshot?key=$CRON_SECRET"` on the VPS — never from a local machine.
7. Verify: `curl -si https://<host>/.well-known/oauth-authorization-server` → 200 JSON,
   no `x-middleware-rewrite` header; `curl -si -X POST https://<host>/api/mcp` → 401 with
   `WWW-Authenticate`.
8. Phase 2 needs no migration; Phase 3 adds `development_snapshots`. After deploying, open a pairing window and reconnect claude.ai once so the consent lists the write tools. Phase 4 adds WhatsApp tools; the first live send should go to a lead you name, with you watching.
...
| Tools return `config` | `MCP_PUBLIC_ORIGIN` invalid, or (for WhatsApp tools) one of the four `OPENWA_*` variables missing or invalid |
