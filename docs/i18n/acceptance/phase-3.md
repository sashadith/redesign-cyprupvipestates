...
- Der Hebräische Styleguide (`docs/i18n/he-styleguide.md`) und das Glossar (`docs/i18n/he-glossary.md`) werden als gecachter System-Block (`heSystemBlock`, `cache_control: ephemeral`) in jeden der drei KI-Generatoren (Description, Area, SEO-Meta) injiziert. **Wer diese beiden Dateien bearbeitet, ändert damit das Verhalten der Generatoren** — aber nicht sofort: `src/lib/ai/heContext.ts` liest beide Dateien einmalig **beim Modul-Load** (`readFileSync` in Top-Level-`const`s, absichtlich die einfachste Variante — siehe die Kommentare im Modul), nicht pro Aufruf. Eine Bearbeitung wirkt sich daher erst nach einem **Prozess-Neustart bzw. Redeploy** aus, nicht sofort beim nächsten API-Call. `readFileSync` ist zusätzlich in try/catch gekapselt: fehlt eine der beiden Dateien (z. B. bei einem unvollständigen Deploy), fällt das Modul auf einen kurzen inline-Fallback-Text zurück und loggt eine `console.warn` mit dem fehlenden Dateinamen, statt beim Start hart zu crashen.
