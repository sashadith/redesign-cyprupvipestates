...
- ~~**`No content (stop: max_tokens)`**~~ — zwei von neun Zeilen im ersten Staging-Lauf: `claude-sonnet-5` denkt standardmäßig adaptiv, die Denk-Tokens zählen gegen `max_tokens`, und mit 4000 war das Budget bei manchen Zeilen aufgebraucht, bevor ein Textblock kam. Behoben: `MAX_TOKENS` 16000 in `translateHe.ts`, Fehlermeldung nennt jetzt die Blocktypen. Betroffene `FAILED`-Zeilen werden über „Enqueue missing" neu eingereiht — **nach** dem Deploy, das die Änderung mitbringt. Dieselbe 4000er-Grenze steht in `projectDescription.ts` und `areaContent.ts` (Bestandscode auf `main`, dort bisher ohne beobachtete Ausfälle) — bei ähnlichen Fehlern zuerst dort schauen.
