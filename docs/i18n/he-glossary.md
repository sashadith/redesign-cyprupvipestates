...
**Stand:** 2026-09-13 · verbindlich für alle `he`-Texte und als Prompt-Kontext für jeden Übersetzungs- und Generierungslauf (Projektbeschreibungen, Meta, Area-Texte). Ergänzungen kommen aus den Lektor-Protokollen (`docs/i18n/reviews/`) und aus `he-keyword-map.md` (die Schreibweise mit dem höheren Suchvolumen gewinnt).
...
| Limassol | לימסול | לימאסול, לימסל | häufigste Suchschreibweise |
| Paphos | פאפוס | פפוס, פאפוס־ | |
...
| new build / new development | פרויקט חדש | Plural פרויקטים חדשים |
...
| developer | יזם / חברה יזמית | Firmennamen lateinisch |
...
| handover | מסירה | |
| price from | החל מ- | `החל מ-€450,000` |
| price on request | מחיר לפי פנייה | |
| sold out | נמכר במלואו | Badge kurz: `נמכר` |
| available | זמין / זמינות | |
| reserved | שמור | |

## 3. Aufenthalt, Steuern, Relocation
...
| relocation | רילוקיישן | Suchbegriff; formeller: `העתקת מגורים` |
| cost of living | יוקר המחיה | |
| international school | בית ספר בינלאומי | |
| healthcare (GESY) | מערכת הבריאות (GESY) | |
...
| Developers | יזמים | |
| Blog / Insights | בלוג | |
| Case studies | סיפורי לקוחות | nicht „מקרי בוחן" |
| About us | עלינו | Slug bleibt `about-us` |
| Contact | צור קשר | Nominal ist hier akzeptierter Standard |
| FAQ | שאלות ותשובות | |
...
## 6. Offene Begriffe (nach Keyword-Daten oder Lektor zu entscheiden)

- `דירות למכירה` vs. `דירות לרכישה` — Volumen entscheidet (Keyword-Map).
