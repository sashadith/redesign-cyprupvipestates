...
| `30 4 * * *` | `drive-sync` | production, `?key=$CRON_SECRET` — Google-Drive developers only; it skips Dropbox- **and** SharePoint-linked accounts by URL (see `kuutio-sync` and `korantina-sync` below) |
...
| `20 3 * * *` | `korantina-sync` (Korantina Homes' SharePoint shared-link sync — see src/lib/sharepointAvailabilitySync.ts) | production, `?key=$CRON_SECRET&scheduled=1` — installed 2026-08-26 together with the adapter. Same `scheduled=1` semantics as `kuutio-sync`: it is what lets Korantina's own `driveSyncInterval` (weekly) skip a run. Slot chosen to sit between `kuutio-sync` (03:00, ~12 min) and `feed-sync` (04:00), still inside `action-digest`'s 4h lookback. `syncAllDrives` skips SharePoint accounts by URL, so this route is the ONLY thing responsible for Korantina's 18 projects — the same gap that left Kuutio unrefreshed for 13 days, closed here on day one. Add `&dry=1` for a read-only run that reports the full folder → table → project mapping and every row it would not import; a dry run writes no `cron_run_logs` row on purpose |
