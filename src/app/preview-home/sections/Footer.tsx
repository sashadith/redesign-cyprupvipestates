...
/* Footer — restyled to the redesign (dark). Reuses getFooterByLang + the
   ...
   The live Footer + module CSS are untouched. */
      ...
      <div className="pf__container">
        <div className="pf__top">
          ...
          <div className="pf__col pf__col--news">
            ...
            <FooterNewsletter placeholder={placeholderFor(lang)} buttonLabel={newsletterButtonLabel} lang={lang} />
          </div>
        </div>
                  ...
                  {col.links?.length > 0 && (
                    <ul>
                      {col.links.map((l: FooterColumnLink) =>
                        l?.label && l?.url ? (
                          <li key={l._key}>
                            <Link href={l.url} {...(l.url.startsWith("http") ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                              {l.label}
                            </Link>
                          </li>
                        ) : null
