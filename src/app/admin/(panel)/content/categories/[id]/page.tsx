...
export default async function EditCategory({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-xl">
      ...
      <form action={save} dir={localeDir(c.language)} className="bg-white rounded-lg border border-[#E5E7EB] p-5 space-y-4">
        <div>
          <label className="block text-sm mb-1">Title</label>
          <input name="title" defaultValue={c.title} className={input} />
        </div>
        <SlugField initialValue={c.slug} language={c.language} />
