// ISO 3166-1 alpha-2 country list — used for the Lead "Country of residence"
// dropdown and the leads-list flag column. Flag emoji is derived from the
// code itself (regional indicator symbols), not stored or imported from an
// external library.
...
export const COUNTRY_NAME_BY_CODE: Record<string, string> = Object.fromEntries(COUNTRIES.map((c) => [c.code, c.name]));

/** ISO 3166-1 alpha-2 code -> flag emoji, via Unicode regional indicator symbols. No image, no library. */
...
// A handful of common free-text spellings that don't exactly match the
// canonical name above — used only for the one-time Nationality ->
// countryOfResidence backfill (Batch A, 2026-07-25), matched case-insensitively.
const NATIONALITY_ALIASES: Record<string, string> = {
  uk: "GB", "united kingdom": "GB", britain: "GB", "great britain": "GB", england: "GB",
  usa: "US", "united states of america": "US", america: "US",
  uae: "AE", russia: "RU", "russian federation": "RU",
...
/** Best-effort match of a free-text nationality string to an ISO code — only used for the one-time backfill, never guesses beyond an exact (case-insensitive) name/alias match. */
export function guessCountryCodeFromNationality(text: string): string | null {
  const norm = text.trim().toLowerCase();
  if (!norm) return null;
  return NAME_TO_CODE[norm] ?? NATIONALITY_ALIASES[norm] ?? null;
}
