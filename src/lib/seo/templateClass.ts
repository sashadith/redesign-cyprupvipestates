...
const HOME = new RegExp(`^/(${L})?$`);
const LISTING = new RegExp(`^(?:/(${L}))?/projects$`);
const PROJECT = new RegExp(`^(?:/(?:${L}))?/projects/([^/]+)$`);
const BLOG = new RegExp(`^(?:/(${L}))?/blog/[^/]+$`);
...
export function templateClassOf(url: string, developmentSlugs?: Set<string>): TemplateClass {
  if (HOME.test(url)) return "homepage";
  if (LISTING.test(url)) return "projects-listing";
